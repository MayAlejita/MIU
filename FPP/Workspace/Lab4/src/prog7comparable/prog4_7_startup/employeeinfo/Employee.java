package prog7comparable.prog4_7_startup.employeeinfo;

import java.util.Date;
import java.util.GregorianCalendar;

import prog7comparable.prog4_7_startup.MyStringList;
...
public class Employee implements Comparable<Employee> { 
	...
	public int compareTo(Employee e) {
		double balanceL = accounts.getLargest().getBalance();
		
		if(balanceL < e.accounts.getLargest().getBalance()) {
			return -1;
		}
		else if(balanceL > e.accounts.getLargest().getBalance()) {
			return 1;
		}
		else { 
			return 0;

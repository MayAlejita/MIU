...
public class MyBST {
	...
	public int[] readIntoArray() {
		Node node = root;
		int bstSize = getSize(root);	
		int[] arr = new int[bstSize];

		ArrayList<Integer> bstList = new ArrayList<>();
		bstList = getOrderArray(node, bstList);

		for (int i = 0; i < bstSize; i++) {
			arr[i] = bstList.get(i).intValue();
		}
	...
	private ArrayList<Integer> getOrderArray(Node node, ArrayList<Integer> arr) {

		if (node != null) {
			getOrderArray(node.left, arr);
			arr.add(node.element);
			getOrderArray(node.right, arr);
		}

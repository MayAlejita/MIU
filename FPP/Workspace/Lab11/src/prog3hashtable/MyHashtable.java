...
public class MyHashtable implements Iterable {
	...
	private void rehash() {
		...
		for (int i = 0; i < sizeTable; i++) {
			if (table[i] != null) {
				tableTemp[i] = new LinkedList();
				Iterator it = table[i].iterator();
				while (it.hasNext()) {
					entry = (Entry) it.next();					
					tableTemp[i].add(entry);
				}
			}
		}
		table = tableTemp;
	...
	/**
	 * returns iterator for the keys only -- best to change to Iterator or Entry
	 * objects
	 */

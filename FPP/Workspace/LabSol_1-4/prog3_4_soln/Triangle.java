...
public final class Triangle {
	 ...
	 * the base and height under the assumption that the base is the longest side 
	...
	public Triangle(double s1, double s2, double s3)  {
		double[] arr = {s1, s2, s3};
		Arrays.sort(arr);		//now arr[0] <= arr[1] <= arr[2]
		double x = arr[0];
		double y = arr[1];
		double z = arr[2];
		if(x + y < z) { //cannot form a triangle with these three sides
		...
		side2 = y;
		side3 = z;
		//compute height h using Pythagorean Theorem
		//two equations, 
